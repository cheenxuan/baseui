...
class UiAdapter(context: Context) : RecyclerView.Adapter<ViewHolder>() {
    ...
    //存储显示的布局item的数据集合
    private var dataSets = ArrayList<UiDataItem<*, out ViewHolder>>()
...
//    private var typedArray = SparseArray<HiDataItem<*, out RecyclerView.ViewHolder>>()
    ...
    fun getOriginalData(): List<UiDataItem<*, out ViewHolder>> {
        return dataSets
    }
     ...
     * 添加多个显示的item
     */
    fun addItems(items: List<UiDataItem<*, out ViewHolder>>, notify: Boolean) {
     ...
     * 添加多个显示的item
     */
    fun addItemsAt(index: Int, items: List<UiDataItem<*, out ViewHolder>>, notify: Boolean = true) {

        items.forEachIndexed { i, dataItem ->
            dataSets.add(index + i, dataItem)
            dataItem.setAdapter(this)
        }

        if (notify) {
            notifyItemRangeChanged(index, dataSets.size)
        }
    }
     ...
     * 通过下标删除item
     */
    fun removeItemAt(index: Int): UiDataItem<*, out ViewHolder>? {
        return if (index >= 0 && index < dataSets.size) {
            val removeData: UiDataItem<*, out ViewHolder> = dataSets.removeAt(index)
     ...
     * 通过item来进行删除
     */
    fun removeItem(dataItem: UiDataItem<*, *>) {
     ...
     * 通过item来进行删除
     */
    fun removeItemsAt(dataItems: List<UiDataItem<*, *>>) {
        dataItems.forEach { 
            removeItem(it)
        }
    }
    ...
    private fun createViewHolderInternal(
        javaClass: Class<UiDataItem<*, out ViewHolder>>,
        view: View
    ): ViewHolder {
        ...
        return object : UiViewHolder(view) {}
    }
    ...
    fun getItem(position: Int): UiDataItem<*, ViewHolder>? {
        if (position < 0 || position >= dataSets.size)
            return null
        return dataSets[position] as UiDataItem<*, ViewHolder>
    }
